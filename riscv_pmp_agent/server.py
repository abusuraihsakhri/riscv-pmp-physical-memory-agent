"""
FastAPI REST API Server for RISC-V Physical Memory Protection (PMP) & ePMP Matrix Agent.
"""
...
def create_app():
    try:
        ...
        app = FastAPI(
            title="RISC-V Physical Memory Protection (PMP) & ePMP Matrix Agent",
            description="Verifies RISC-V Machine-mode PMP address match registers (TOR, NA4, NAPOT) and locked execution configurations.",
        ...
        @app.get("/health")
        def health():
            return {"status": "HEALTHY", "system": "riscv-pmp-physical-memory-agent", "domain": "Hardware Security", "version": "2.0.0-FRONTIER"}
